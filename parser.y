...
proc_stmt: ID {$$ = new ProcStmt(ProcStmt::T_SIMPLE, *$1);}
...
expression: expression  GE  expr {$$ = new Expression(Expression::T_GE, $1, $3);}
                  ...
                  |  expression  LE  expr  {$$ = new Expression(Expression::T_LE, $1, $3);}
                  |  expression  LT  expr  {$$ = new Expression(Expression::T_LT, $1, $3);}
...
term: term  MUL  factor  {$$ = new Term(Term::T_MUL, $1, $3);}
                  |  term  DIV  factor  {$$ = new Term(Term::T_MUL, $1, $3);}
                  |  term  MOD  factor  {$$ = new Term(Term::T_MUL, $1, $3);}
                  |  term  AND  factor  {$$ = new Term(Term::T_MUL, $1, $3);}
                  |  factor {$$ = new Term($1);}
factor: NAME  {$$ = new Factor(Factor::T_NAME, *$1);}
                  |  NAME  LP  args_list  RP  {$$ = new Factor(Factor::T_NAME_ARGS, *$1, $3);}
                  |  SYS_FUNCT {$$ = new Factor(Factor::T_SYS_FUNCT, *$1);}
                  |  SYS_FUNCT  LP  args_list  RP  {$$ = new Factor(Factor::T_SYS_FUNCT_ARGS, *$1, $3);}
